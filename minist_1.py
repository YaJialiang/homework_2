...
def extract_features(X, down_size=14):
    """灰度降采样 + 边缘方向图特征提取"""
    features = []
    for img in X:
        img = img.reshape(28, 28)  # 将一维数组重塑为28x28的二维数组
        # 灰度降采样
        gray_small = zoom(img, (down_size/28, down_size/28))

        # Sobel 边缘检测
        sx = sobel(img, axis=1)
        sy = sobel(img, axis=0)
        edge = np.sqrt(sx**2 + sy**2)
        edge /= np.max(edge) + 1e-8  # 归一化
        edge_small = zoom(edge, (down_size/28, down_size/28))

        # 拼接特征：2 * 14 * 14 = 392
        feat = np.concatenate([gray_small.flatten(), edge_small.flatten()])
...
def main():
    ...
    X_train_val, X_test, y_train_val, y_test = train_test_split(
        X_feat[:9000], y[:9000], test_size=2000, random_state=42
    )
    ...
    X_val = X_train_val[5000:7000]
    y_val = y_train_val[5000:7000]
