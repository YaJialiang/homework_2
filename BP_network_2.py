...
class BPNetwork:
    ...
    def __init__(self, n_input=2, n_hidden=1, n_output=1, seed=None, output_activation='sigmoid'):
        if seed is not None:
            np.random.seed(seed)
...
if __name__ == '__main__':
    ...
    net = BPNetwork(n_input=2, n_hidden=3, n_output=1, seed=1, output_activation='sigmoid')
    print('Initial weights (w1):\n', net.w1)
    print('Initial weights (w2):\n', net.w2)

    net.train(X, T, epochs=10000, eta=1.0, verbose=True, print_every=2000)

    preds = net.predict(X)
    print('\nFinal predictions:')
    for x, t, p in zip(X, T, preds):
        print(f"x={x}, target={int(t[0])}, pred={p[0]:.4f}")
